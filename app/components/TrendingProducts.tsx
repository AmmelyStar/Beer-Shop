...
export default function TrendingProducts({
  ...
  return (
    <section className="mx-auto max-w-2xl px-4 pt-6 pb-26 sm:px-6 lg:max-w-7xl lg:px-8">
      ...
      <div className="mt-8 grid grid-cols-1 gap-y-12 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8 items-stretch">
        {products.map((product) => {
          const abv = product.specs?.abv ? Number(product.specs.abv) : undefined;

          const priceObj = product.priceRange?.minVariantPrice;
          const price =
            priceObj != null
              ? `${Number(priceObj.amount).toFixed(2)} ${priceObj.currencyCode}`
              : "";
          ...
          return (
            <div key={product.id} className="group flex flex-col h-full">
              ...
              <div className="relative flex-1 flex flex-col">
                {/* ✅ БЕЛЫЙ ФОН ДЛЯ КАРТИНКИ (всегда) */}
                <div className="relative aspect-square w-full overflow-hidden rounded-lg bg-white border border-black/5">
                  <Image
                    src={
                    ...
                    className="object-contain transition-transform duration-300 transform-gpu will-change-transform group-hover:scale-105 p-4"
                  />
                ...
                <div className="mt-4 flex items-start justify-between">
                  <div>
                    ...
                    {abv !== undefined && (
                      <p className="text-sm text-neutral-300">{abv} %</p>
                    )}
                  </div>
